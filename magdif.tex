...
This results in (\textbf{TODO: check sign of $\boldsymbol{A}_{n}$})
\begin{align*}
\boldsymbol{j}_{n}\cdot\nabla\psi & =-\left(in\,p_{n}-\boldsymbol{j}_{0}\cdot(\boldsymbol{B}_{n}\times\boldsymbol{e}_{\ph})\right)\\
 & =-\left(p_{n}+\boldsymbol{j}_{0}^{\text{pol}}\cdot\boldsymbol{A}_{n}\right),
\end{align*}
where $\boldsymbol{A}_{n}$ is the fully poloidal vector potential
